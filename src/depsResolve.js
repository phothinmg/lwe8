 ...
 */
export function depsResolve({ dependency, filename, directory } = {}) {
...
function getDependencyPath(dependency, filename, directory) {
  if (dependency.startsWith("..") || dependency.startsWith(".")) {
    return resolve(dirname(filename), dependency);
  }

  return resolve(directory, dependency);
}
...
function getDependencyExtension(dependency, filename) {
  const depExt = extname(dependency);
